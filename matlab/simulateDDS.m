...
for nn = 2:N
    ...
    dds3(nn) = mod(dds3(nn-1) + floor(8*df/clk*2^(phaseWidth)),2^phaseWidth);
end
...
dds3r = double(dds3)*2^(outputWidth - phaseWidth);
% plot(dds1r,'.-');
% hold on;
plot(floor(dds1r)-floor(dds2r),'.-');
% plot(dds3r,'.-');
